...
def _lire_docx(contenu):
    ...
    for tbl in doc.tables:
        for row in tbl.rows:
            morceaux.append(" | ".join(c.text for c in row.cells))
    return "\n".join(morceaux)
...
def lire_fichiers(items, ocr=True, workers=4, progression=None):
    """Plusieurs fichiers (contenu_bytes, nom) : PDF texte, PDF scanné, image, .docx, .txt.
    Les pages sans texte (scans/photos) sont toutes lues EN PARALLÈLE par le modèle de vision,
    ...
    for contenu, nom in items:
        ...
        try:
            if ext == "docx":
                pages.append({"doc": nom, "page": 1, "texte": _lire_docx(contenu), "ocr": False, "erreur": ""})
                continue
            if ext == "txt":
                pages.append({"doc": nom, "page": 1, "texte": contenu.decode("utf-8", errors="ignore"),
                              "ocr": False, "erreur": ""})
                continue
            doc = fitz.open(stream=contenu, filetype="pdf" if ext == "pdf" else ext)
        except Exception as e:
            pages.append({"doc": nom, "page": 1, "texte": "", "ocr": False, "erreur": f"Fichier illisible : {e}"})
            continue
        docs_ouverts.append(doc)
        for i, p in enumerate(doc):
            try:
                texte = p.get_text().strip() if ext == "pdf" else ""
            except Exception:
                texte = ""
            entree = {"doc": nom, "page": i + 1, "texte": texte, "ocr": False, "erreur": ""}
            if len(texte) < 40 and ocr:
                entree["_page"] = p  # référence temporaire, retirée avant de renvoyer
            pages.append(entree)

    a_lire = [e for e in pages if "_page" in e]
    if a_lire:
        faits = 0
        with ThreadPoolExecutor(max_workers=workers) as pool:
            futurs = {pool.submit(ocr_page, e["_page"]): e for e in a_lire}
            for f in as_completed(futurs):
                e = futurs[f]
                try:
                    e["texte"], e["ocr"] = f.result(), True
                except Exception as exc:
                    e["erreur"] = str(exc)[:200]
                faits += 1
                if progression:
                    progression(faits / len(a_lire))
    for e in pages:
        e.pop("_page", None)
    for d in docs_ouverts:
        d.close()
    return pages
...
def extraire_exigences(pages, taille_bloc=4, progression=None, workers=4):
    blocs = [pages[i:i + taille_bloc] for i in range(0, len(pages), taille_bloc)]
    textes = ["\n\n".join(f"[Page {p['page']}]\n{p['texte']}" for p in b) for b in blocs]

    def traiter(texte):
        return appel_json(PROMPT_EXIGENCES, texte[:14000]) if len(texte.strip()) >= 50 else []

    resultats, faits = [None] * len(textes), 0
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futurs = {pool.submit(traiter, t): i for i, t in enumerate(textes)}
        for f in as_completed(futurs):
            i = futurs[f]
            try:
                resultats[i] = f.result()
            except Exception:
                resultats[i] = []
            faits += 1
            if progression:
                progression(faits / len(textes))
...
def verifier_toutes(exigences, index, progression=None, workers=4):
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futurs = {pool.submit(verifier, ex, index): i for i, ex in enumerate(exigences)}
        for f in as_completed(futurs):
            i = futurs[f]
            try:
                res[i] = {**exigences[i], **f.result()}
            except Exception as e:
                res[i] = {**exigences[i], "verdict": "manquant",
                          "justification": f"Erreur technique : {e}", "citation": "", "action": ""}
            faits += 1
            if progression:
                progression(faits / len(exigences))
    return res
