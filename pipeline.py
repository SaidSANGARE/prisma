...
def ocr_page(page):
    """Lit une page (scan ou photo) avec le modèle de vision. Essaie 2 tailles et 2 formats d'envoi."""
    derniere = None
    for max_dim, qualite in ((1400, 70), (900, 55)):
        b64 = base64.b64encode(_render_jpeg(page, max_dim, qualite)).decode()
        url = f"data:image/jpeg;base64,{b64}"
        essais = [
            [{"type": "text", "text": PROMPT_OCR}, {"type": "image_url", "image_url": {"url": url}}],
            f'{PROMPT_OCR}\n<img src="{url}" />',
        ]
        for contenu in essais:
            try:
                rep = _client().chat.completions.create(
                ...
                derniere = e
    raise RuntimeError(f"Lecture d'image impossible : {derniere}")
...
def plan_action(resultats, jours_restants=None):
    ...
    textes = {r["id"]: r["texte"] for r in a_traiter}
    for a in plan:
        a["exigence"] = textes.get(a.get("id"), "")
    plan.sort(key=lambda a: a["priorite"] if isinstance(a.get("priorite"), int) else 99)
    return plan
